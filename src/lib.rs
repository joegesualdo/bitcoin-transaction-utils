pub fn is_transaction_hex_segwit(transaction_hex: &String) -> bool {
    fn get_5th_byte(hex: &String) -> String {
        let format = format!(
            "{}{}",
            hex.chars().nth(8).unwrap(),
            hex.chars().nth(9).unwrap()
        );
        format
    }
    let fifth_byte = get_5th_byte(transaction_hex);
    fifth_byte == "00"
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn it_works() {
        let segwit_transaction_hex = "".to_string();
        let non_segwit_transaction_hex = "".to_string();
        assert_eq!(is_transaction_hex_segwit(&segwit_transaction_hex), true);
        assert_eq!(
            is_transaction_hex_segwit(&non_segwit_transaction_hex),
            false
        );
    }
}
